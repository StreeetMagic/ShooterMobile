...
namespace Infrastructure.ZenjectFactories
{
  public abstract class ZenjectFactory : IZenjectFactory
  {
    ...
    public ZenjectFactory(AssetProvider assetProvider, IInstantiator instantiator, ArtConfigProvider artConfigProvider)
    {
    ...
    public GameObject InstantiateGameObject(PrefabId prefabId) => _instantiator.InstantiatePrefab(_artConfigProvider.GetPrefab(prefabId));
    public GameObject InstantiateGameObject(PrefabId prefabId, Transform parent) => _instantiator.InstantiatePrefab(_artConfigProvider.GetPrefab(prefabId), parent);
    public GameObject InstantiateGameObject(PrefabId prefabId, Vector3 position, Quaternion quaternion, Transform parent) => _instantiator.InstantiatePrefab(_artConfigProvider.GetPrefab(prefabId), position, quaternion, parent);
